            ...
            while modoDeBusqueda != 3:
                ...
                if indice != 0:
                    with open("DB\Movimiento.csv", "r", encoding = 'utf8') as documento:
                        lector = csv.DictReader(documento)
                        for linea in lector: 
                            if int(linea['ID_movimiento']) == indice and int(linea['ID_tipo_de_movimiento']) == movimiento_contrario and int(linea['ID_almacén']) == ID_almacen:                                 
                                movimiento[0] = indice
                                movimiento[1] = ID_tipo_de_movimiento
                                movimiento[2] = ID_almacen
                                movimiento[3] = abs(float(linea['Cantidad']))
                                movimiento[4] = abs(float(linea['Importe Total']))
                                movimiento[5] = str(datetime.datetime.now().date())
                                movimiento[6] = str(datetime.datetime.now().time())
                                movimiento[7] = linea['ID_socio']
                                movimiento[8] = opcion
                                movimiento[9] = linea['ID_tipo_de_pago']
                                movimiento[10] = linea['ID_método_de_pago']
                                nuevoArchivo = movimientoTrasAgregar(movimiento)
                                guardarModificaciones("Movimiento.csv", nuevoArchivo)
                        ...
                        with open("DB\Articulo.csv", "r", encoding = 'utf8') as documento: 
                            ...
                            for linea in lector: 
                                if int(linea['ID_articulo']) in articulos:
                                    linea['Costo de Reposición'] = nuevosCostos[articulos.index(int(linea['ID_articulo']))] # ERROR ENCONTRADO
                                    linea['Precio de Venta sin IVA'] = nuevosCostos[articulos.index(int(linea['ID_articulo']))] * (1 + int(find('Utilidad.csv', 'ID_utilidad', linea['ID_utilidad'])[1])/100)
                                    contadorDeCostos += 1
        ...
        for fila, linea in enumerate(lector):
            if fila != 0: 
                if int(linea[0]) == articulo and int(linea[1]) == almacen:
                    ...
                    nuevoArchivo.append(linea)
    if añadido == False:
        nuevoArchivo.append([articulo, almacen, cantidad])
