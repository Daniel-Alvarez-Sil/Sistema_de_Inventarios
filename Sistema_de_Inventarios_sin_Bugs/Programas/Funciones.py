...
def opciones(carpeta : str, archivo : str, seleccion : str, indice : list, regresar : bool, almacen : int = 0) -> list:
    ...
    contador = 0
    comillas = '"'
    campos = titulos(carpeta, archivo)
            ...
            if fila != 0:
                if almacen == 0: 
                    ...
                    for i in indice: 
                        if i != indice[0]:
                            cOpcion += '\t\t'
                        cOpcion += f"{campos[i].replace(comillas, '')}: {linea[i].replace(comillas, '')}"
                        if i != indice[len(indice) - 1]:
                            cOpcion += '\n'
                        elif len(indice) > 1:
                            cOpcion += '\n'
                    print(cOpcion)
...
def obtenerAlmacen(carpeta : str = "DB", archivo : str = "Almacén.csv", seleccion : str = "almacén", indice : list = [1], regresar : bool = True) -> int:
    return opciones(carpeta, archivo, seleccion, indice, regresar)

def obtenerModulo(carpeta : str = "Menus", archivo : str = "Menú Principal.csv", seleccion : str = "módulo", indice : list = [1], regresar : bool = True) -> int:
    return opciones(carpeta, archivo, seleccion, indice, regresar)

def obtenerOpcionInicial(carpeta : str = "Menus", archivo : str = "Menú Inicial.csv", seleccion : str = "apartado", indice : list = [1], regresar : bool = False) -> int:
    return opciones(carpeta, archivo, seleccion, indice, regresar)

def obtenerOpcionArticulos(carpeta : str = "Menus", archivo : str = "Menú de Artículos.csv", seleccion : str = "apartado de artículos", indice : list = [1], regresar : bool = True) -> int:
    return opciones(carpeta, archivo, seleccion, indice, regresar)

def obtenerOpcionSociosDeNegocios(carpeta : str = "Menus", archivo : str = "Menú de Socios de Negocios.csv", seleccion : str = "apartado de socio de negocios", indice : list = [1], regresar : bool = True) -> int:
    return opciones(carpeta, archivo, seleccion, indice, regresar)

def obtenerOpcionEmpleados(carpeta : str = "Menus", archivo : str = "Menú de Empleados.csv", seleccion : str = "apartado de empleados", indice : list = [1], regresar : bool = True) -> int:
    return opciones(carpeta, archivo, seleccion, indice, regresar)

def obtenerOpcionReportes(carpeta : str = "Menus", archivo : str = "Menú de Reportes.csv", seleccion : str = "apartado de reportes y consultas", indice : list = [1], regresar : bool = True) -> int:
    return opciones(carpeta, archivo, seleccion, indice, regresar)
...
def guardarModificaciones(archivo : str, modificaciones : list, carpeta : str = 'DB') -> None:
    headers = titulos(carpeta, archivo)
    with open(f'{carpeta}\{archivo}', "w", newline = '', encoding = "utf8") as documento:
...
def find(archivo : str, campoCondicion : str, condicion : str) -> list:
